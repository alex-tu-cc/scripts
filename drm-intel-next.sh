...
url='https://kernel.ubuntu.com/~kernel-ppa/mainline/drm-intel-next'
script="$0"
...
download_and_install_kernels ()
{
    ...
    for ver in $(eval echo $downloads); do
        ver="${ver/~rc/-rc}"
        debver="${ver/-rc/~rc}"
        pkgs=`wget -q $url/$ver/ -O - | grep -o "linux[^\"]*\(all\|$arch\).deb" | grep -v -e lowlatency -e cloud | sort -u`
        for pkg in $pkgs; do
            [ -d "$PWD/kernels/$debver" ] || mkdir -p "$PWD/kernels/$debver"
            [ -s "$PWD/kernels/$debver/$pkg" ] || wget -nv --show-progress "$url/$ver/$arch/$pkg" -O "$PWD/kernels/$debver/$pkg"
            [ -s "$PWD/kernels/$debver/$pkg" ] || wget -nv --show-progress "$url/$ver/$pkg" -O "$PWD/kernels/$debver/$pkg"
        done
        if [ -z "$pkgs" ]; then
            echo "There is no ${debver} mainline kernel to download."
        elif [ -z "$download_only" ]; then
            sudo dpkg -i $PWD/kernels/$debver/*.deb
        fi
    done
}
...
{
    vers="current `wget -q $url -O - | grep -o 'href="[0-9][^"]*"' | grep -o '[0-9][^/]*' | sort -V`"
